...
module.exports = function(options) {
    ...
    return () => {
        gulp.watch(
            `./${options.src}/js/**/*`,
            gulp.series(options.tasks.buildCustomJs, options.tasks.esLint)
        );

        gulp.watch(
            `./${options.src}/scss/**/*`,
            gulp.series(options.tasks.buildSass)
        );

        gulp.watch(
            `./${options.src}/html/**/*`,
            gulp.series(options.tasks.fileInclude)
        );

        const imagesWatcher = gulp.watch(
            `./${options.src}/images/**/*.+(${options.imageExtensions})`
        );
